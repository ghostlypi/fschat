...
say "Then register once — the daemon installs itself as an always-on user service"
say "(auto-starts on login/boot, restarts on crash; no 'start' needed):"
say "  fschat-daemon register <username>"
say "  fschat-daemon create <user>        # then open the .chat file it prints with vim"
say "  (manage it with: systemctl --user status|restart|stop fschat.service)"
